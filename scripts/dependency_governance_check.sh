...
run_workspace_checks() {
  ...
  local outdated_args=(--root-deps-only --depth 1)
  for dep in "${OUTDATED_IGNORE_DEPS[@]}"; do
    outdated_args+=(--exclude "${dep}")
  done
  if [[ "${STRICT_OUTDATED}" == "1" ]]; then
    (
      cd "${workspace_path}"
      run_cmd cargo outdated "${outdated_args[@]}" --exit-code 1
    )
  else
    (
      cd "${workspace_path}"
      run_cmd cargo outdated "${outdated_args[@]}"
    )
  fi
}
